...
class InquirerPyFuzzyControl(InquirerPyUIListControl):
	...
	def _get_hover_text(self, choice) -> List[Tuple[str, str]]:
		...
		if not choice["indices"]:
			if self.choices[choice["index"]]["enabled"]: ## <<<< color selection
				display_choices.append(("class:marker", choice["name"]))
			else:
			...
			for index, char in enumerate(choice["name"]):
				...
				elif self.choices[choice["index"]]["enabled"]: ## <<<< color selection
					display_choices.append(("class:marker", char))
				else:
					display_choices.append(("class:pointer", char))

		if "instruction" in choice and choice["instruction"]: ## <<<< color instruction
			display_choices.append(
	...
	def _get_normal_text(self, choice) -> List[Tuple[str, str]]:
		...
		if not choice["indices"]:
			if self.choices[choice["index"]]["enabled"]: ## <<<< color selection
				...
				display_choices.append(("class:choice_name", choice["name"])) 
		else:
			indices = set(choice["indices"])
			for index, char in enumerate(choice["name"]):
				if index in indices:
					...
					display_choices.append(("class:choice_name", char)) # <<<< color name
	...
	async def _filter_choices(self, wait_time: float) -> List[Dict[str, Any]]:
		...
		if not self._current_text():  # If search buffer is empty
			...
			for choice in self.choices:
				...
				# combining 'name' and 'instruction'
				search_text_parts = [choice.get("name", "")]
